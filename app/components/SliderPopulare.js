    ...
    <section style={{ background: "#EEF2EC", padding: isMobile ? "48px 16px" : "72px 16px", overflow: "hidden" }}>
      <div style={{ maxWidth: "var(--container-inner)", margin: "0 auto" }}>
        <div style={{ marginBottom: 24 }}>
          <h2 style={{ fontSize: "clamp(28px, 3.5vw, 56px)", fontWeight: 900, lineHeight: 1.1, margin: "0 0 6px", color: "#1D2820" }}>Produse Populare</h2>
        ...
        <div style={{ display: "flex", justifyContent: "center", alignItems: "center", gap: 12, marginTop: 32, flexWrap: "wrap" }}>
          ...
          >&#8249;</button>
          <Link href="/populare" style={{ padding: "12px 36px", border: "2px solid #2C662D", borderRadius: 999, fontSize: 14, fontWeight: 700, color: "#2C662D", textDecoration: "none", transition: "all 0.2s", background: "transparent" }}
            onMouseEnter={(e) => { e.currentTarget.style.background = "#214F27"; e.currentTarget.style.color = "#fff"; }}
            onMouseLeave={(e) => { e.currentTarget.style.background = "transparent"; e.currentTarget.style.color = "#2C662D"; }}
          >Vezi toate</Link>
          <button
