...
export default function SliderPopulare() {
  ...
  useEffect(() => {
    fetch("/api/produse")
      .then((r) => r.json())
      .then((data) => {
        const filtered = Array.isArray(data) ? data.filter((p) => p.tags?.includes("populare")) : [];
  ...
  return (
    <section style={{ background: "#f5f3f0", padding: isMobile ? "48px 16px" : "72px 24px", overflow: "hidden" }}>
      <div style={{ maxWidth: "var(--container)", margin: "0 auto" }}>
        <div style={{ marginBottom: 24 }}>
          <h2 style={{ fontSize: "clamp(28px, 3.5vw, 56px)", fontWeight: 900, lineHeight: 1.1, margin: "0 0 6px", color: "#111" }}>Produse Populare</h2>
          <p style={{ fontSize: "clamp(13px, 1.1vw, 17px)", color: "#666", margin: 0, lineHeight: 1.4 }}>Cele mai apreciate produse din colecția noastră</p>
        </div>
        ...
        <div style={{ display: "flex", justifyContent: "center", alignItems: "center", gap: 12, marginTop: 32, flexWrap: "wrap" }}>
          ...
          >&#8249;</button>
          <Link href="/populare" style={{ padding: "12px 36px", border: "2px solid #2B652C", borderRadius: 999, fontSize: 14, fontWeight: 700, color: "#2B652C", textDecoration: "none", transition: "all 0.2s", background: "transparent" }}
            onMouseEnter={(e) => { e.currentTarget.style.background = "#2B652C"; e.currentTarget.style.color = "#fff"; }}
            onMouseLeave={(e) => { e.currentTarget.style.background = "transparent"; e.currentTarget.style.color = "#2B652C"; }}
          >Vezi toate</Link>
          <button
