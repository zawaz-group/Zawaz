  ...
  return (
    <>
      ...
      <main style={{ minHeight: "80vh", maxWidth: "var(--container)", margin: "0 auto", padding: "var(--section-padding)" }}>
        <h1 style={{ fontSize: 28, fontWeight: 800, color: "#1D2820", marginBottom: 32, textAlign: "center" }}>Caută produse</h1>
        ...
        <div style={{ maxWidth: 560, margin: "0 auto 48px", position: "relative" }}>
          <input
            ...
            style={{
              ...
              border: "2px solid #DCE4D9", borderRadius: 12, outline: "none",
              boxSizing: "border-box", transition: "border-color 0.2s",
              fontFamily: "inherit", color: "#1D2820"
            }}
            onFocus={e => e.target.style.borderColor = "#1D2820"}
            onBlur={e => e.target.style.borderColor = "#DCE4D9"}
          />
          <svg style={{ position: "absolute", right: 14, top: "50%", transform: "translateY(-50%)", color: "#5D695F", pointerEvents: "none" }}
        ...
        {query.trim().length > 1 && rezultate.length === 0 && (
          <p style={{ textAlign: "center", color: "#5D695F", fontSize: 15 }}>Niciun produs găsit pentru „{query}".</p>
        )}

        {rezultate.length > 0 && (
          <>
            <p style={{ color: "#5D695F", fontSize: 13, marginBottom: 24 }}>{rezultate.length} {rezultate.length === 1 ? "produs găsit" : "produse găsite"}</p>
        ...
        {query.trim().length <= 1 && (
          <div style={{ textAlign: "center", color: "#5D695F", marginTop: 60 }}>
